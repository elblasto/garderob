			...
			if ($status) {
				...
				if ((string)$cost != '') {
					$quote_data['by_total_' . $result['geo_zone_id']] = array(
						...
						'title'        => $result['description'],//$result['name'] . '  (' . $this->language->get('text_total') . ' ' . $this->currency->format($this->tax->calculate($cart_total, $this->config->get('by_total_tax_class_id'), $this->config->get('config_tax'))) . ')',
						'cost'         => $cost,

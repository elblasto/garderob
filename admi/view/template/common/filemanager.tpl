...
$(document).ready(function() { 
	...
	$('#column-right a').live('click', function( e ) {
		if ($(this).attr('class') == 'selected') {
			...
			var $parent = $( this ).closest('#column-right'),
				from	= $parent.find('a.selected').first().index(),
				to		= $( this ).index();

			$parent.find('div > a').slice( from, to + 1 ).addClass('selected');
			IS_SHIFT = true;
		} else {			
			if( IS_SHIFT ) {
